...
WEBHOOK_SIGNATURE_SECRET = b"7Rq66T_WDJJPCBQM0wV_SFqmJU2y6w4kOvf1JwwY2-Y="
...
def verify_webhook_signature(message_body: str):
    ...
    if signature != generate_wsocket_signature(message_body):
        return False
    return True


def generate_wsocket_signature(message_body: json):
...
def send_message_to_socket(message: json):
    wsocket_signature = generate_wsocket_signature(message_body=message)  #signature verification is dropped for now at the server side
    message["signature"] = wsocket_signature
    message_string = json.dumps(message)
    ws.send(message_string)


# on connection:
send_message_to_socket(message={"command": "register"})


#NOTE: A random id genereated by client will be useful, not send duplicate messages to the same client
def process_socket_message(message_json: json):
    #TODO - call the corresponding minesweeper method based on the input recieved here.
    message_json = json.loads(message_json)
    
    if not message_json.get("accept"):
        return False
    ...
    if command == "register":
        ...
        print(f"current player id has been successfully set!: {CURRENT_PLAYER_ID}")
        
    elif command == "start":
        MATCH_STARTED = True
        print(f"Match has been allowed to start!")
    
    elif command == "game_over":
        winner = message_json.get("player_id")
        tkMessageBox.showinfo(title="Match is Over!!", message=f"This match has been concluded, the winner is: {winner}")
        time.sleep(3)
        sys.exit()
...
def run_websocket_forever():
    while True:
        ...
        process_socket_message(message_json)
        ...
        #     send_message_to_socket(error_message)
...
class Minesweeper:
    ...
    def onClick(self, tile):
        # This'll get updated from websocket
        if not MATCH_STARTED:
            tkMessageBox.showerror(title="Searching!",message="Please wait for some time, we're searching for players!")
            return
